...
use scv_node::{opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		scv_node::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		scv_node::native_version()
	}
}
...
pub fn create_extrinsic(
	...
	function: impl Into<scv_node::Call>,
	nonce: Option<u32>,
) -> scv_node::UncheckedExtrinsic {
	...
	let period = scv_node::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: scv_node::SignedExtra = (
		// frame_system::CheckNonZeroSender::<scv_node::Runtime>::new(),
		frame_system::CheckSpecVersion::<scv_node::Runtime>::new(),
		frame_system::CheckTxVersion::<scv_node::Runtime>::new(),
		frame_system::CheckGenesis::<scv_node::Runtime>::new(),
		frame_system::CheckMortality::<scv_node::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<scv_node::Runtime>::from(nonce),
		frame_system::CheckWeight::<scv_node::Runtime>::new(),
		pallet_transaction_payment::ChargeTransactionPayment::<scv_node::Runtime>::from(tip),
	);

	let raw_payload = scv_node::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			// (),
			scv_node::VERSION.spec_version,
			scv_node::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	scv_node::UncheckedExtrinsic::new_signed(
		function.clone(),
		sp_runtime::AccountId32::from(sender.public()).into(),
		scv_node::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
...
// 					frame_system::CheckTxVersion::<scv_node::Runtime>::new(),
// 					frame_system::CheckGenesis::<scv_node::Runtime>::new(),
// 					frame_system::CheckMortality::<scv_node::Runtime>::from(generic::Era::Immortal),
// 					frame_system::CheckNonce::<scv_node::Runtime>::from(index),
// 					frame_system::CheckWeight::<scv_node::Runtime>::new(),
// 					pallet_transaction_payment::ChargeTransactionPayment::<scv_node::Runtime>::from(
// 						tip,
// 					),
// 				);
