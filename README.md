...
```bash
./target/release/scv-chain --dev
```

Purge the development chain's state:

```bash
./target/release/scv-chain purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/scv-chain -ldebug --dev
```

> Development chain means that the state of our chain will be in a tmp folder while the nodes are
> running. Also, **alice** account will be authority and sudo account as declared in the
> [genesis state](https://github.com/substrate-developer-hub/substrate-scv-chain/blob/main/node/src/chain_spec.rs#L49).
...
$ ./target/release/scv-chain --dev --base-path ./my-chain-state/
...
### Deploy node on server

First, install [Docker](https://docs.docker.com/get-docker/) and
[Docker Compose](https://docs.docker.com/compose/install/).

- Connect to test environment
- Redirect to substrate node directory

  ```bash
    cd /home/ec2-user/scv/substrate-node
  ```

- Pull new source code from github with target branch

  ```bash
    ./scripts/docker_run.sh
  ```

Then wait until the image finishes building

- Optional: Remove used docker images: docker image prune
- Check if containers are running, run `docker ps` then check the results
- Try connecting the link to verify the web is up and running: https://talio-net.com
